...
% % Without inbuilt trapizoidal integration function
% % trapizoidal function of integration manually
% Cn=zeros(1,32); % fourier series coeff
% j=1;
% d=0;
% c=zeros(1,length(k));
% %trapizoidal function of integration
%     for m = -16:1:16 % Number of fourier series coefficients
%         d=0;
%         c = f.* exp( (-1i*2*m*pi.*k)./T);
%         for v = 1:1:length(k)
%             if (v == 1) || (v == length(k))
%                 d = d + c(v);
%             else
